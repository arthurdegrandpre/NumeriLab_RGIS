...
Most would agree that data science should always start and end with data visualization. It allows scientist to understand the data, its structure and distribution, but also to connect with the public, by creating easily understandable content that facilitates communication. 
...
... and all of this makes it hard to leave the R environment! Still, most of us still are leaving the comfort of R whenever we are faced with spatial data, splitting our workflows and creating different scripts, different processes and ultimately resulting in a loss of time and efficiency.  
...
Previous version of this workshop were mostly focused on understanding basic data structure for spatial objects from the "sp" package, and how they can be used for basic mapping and building interactive maps using leaflet.  
...
webshot::install_phantomjs() # install once
```
...
If you have a shapefile of your data, it can be imported directly as a spatial object by using the function st_read from *sf* or readOGR() from the *rgdal* package. The later will generate an *sp* compatible object.
...
# base R has some functions that allow plotting rasters
...
It is also possible to visualise multiple single band rasters at once using the leafsync package
...
To do so, let's redefine our colors, add layer control and some additionnal information when clicking on points, then let's save the map into an html object.
...
There are many ways to share spatial data without loosing it's spatial properties (CRS, for example).
